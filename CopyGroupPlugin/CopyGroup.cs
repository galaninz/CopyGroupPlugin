    ...
    public class CopyGroup : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                Reference reference = uiDoc.Selection.PickObject(ObjectType.Element, filter, "Выберите группу объектов");
                ...
                XYZ point = uiDoc.Selection.PickPoint("Выберите точку в комнате");
                Room selectedRoom = GetRoomByPoint(doc, point);
                XYZ pointCenter = GetElementCenter(selectedRoom);
                XYZ offsetCenter = pointCenter - offset;
